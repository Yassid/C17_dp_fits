...
\title{\textbf{Spectroscopy of unbound \Cse\ states via the \Cs\dpr\Cse\
...
\Cs\dpr\Cse\ transfer reaction at $11.77$\mevu, measured with the AT-TPC at
...
systematic. Results are compared to the previous in-house \Cs\dpr\ analysis of
...
studied previously in \Cs\dpr\ transfer~\cite{PereiraLopez2020,MovillaAyyad2026};
...
tracking detector. Recoil protons from \Cs\dpr\Cse\ were reconstructed
...
\subsection{Vertex-$z$ slicing and the target-thickness factor}
\label{sec:znorm}
$N_{\rm target}=0.019632$ is the \emph{full-detector} deuteron areal density:
D$_2$ at $300$~Torr ($n_{\rm D}\approx1.98\times10^{19}$~cm$^{-3}$) integrated
over the full active length $z\in[0,0.99]$~m, i.e.\ a per-metre areal density
since the detector is $\approx1$~m long. When the yields are taken from a
restricted vertex-$z$ slice $[z_{\rm lo},z_{\rm hi}]$ rather than the full
length, only that fraction of target contributes, so the cross section carries
an explicit factor $1/\Delta z$ with $\Delta z=z_{\rm hi}-z_{\rm lo}$ in metres,
\begin{equation}
  \frac{\mathrm{d}\sigma}{\mathrm{d}\Omega}
  = \frac{Y}{N_{\rm beam}\,N_{\rm target}\,\Delta\Omega}\cdot 10
    \ \Big/\ \Delta z .
  \label{eq:znorm}
\end{equation}
The earlier in-house bound-state extraction proceeds slice-by-slice, so the
divisor is the \emph{slice thickness in metres}---not an excitation-energy bin
width:
\begin{center}
  \begin{tabular}{ccc}
    \toprule
  ...
  \end{tabular}
\end{center}
The bound states are extracted from the $[0.9,1.0]$~m slice ($/0.1$). The
present unbound analysis instead integrates the \emph{full} vertex-$z$ range
with the full $N_{\rm target}$ and uses full fitted peak areas, so $\Delta z=1$
and no slice factor enters.
...
DWBA sits $\sim$14\,\% below the data. We stress that the large reduced
$\chi^2$ in Table~\ref{tab:omp} ($\chi^2/\nu=32$--$398$) reflect a mismatch in
the \emph{angular shape}---none of the potentials reproduces the full
Coulomb--nuclear diffraction pattern out to large angles---and not a failure of
the absolute scale: the quantity used here is the error-weighted
forward-lobe ratio $\langle\mathrm{data}/\mathrm{OM}\rangle$, which is robust
against the shape mismatch because the forward lobe is Coulomb-dominated and
common to all three potentials. The honest normalization uncertainty is
therefore the \emph{cross-potential spread} of that ratio, $\approx0.86$--$0.98$
...
\paragraph{Consistency of the sliced data with the full-range elastic.}
The elastic distribution is built from the \emph{full} vertex-$z$ range, so the
OM match above certifies $N_{\rm beam}\,N_{\rm target}$ \emph{at the full
thickness} from first principles---the OM cross section is absolute, with no
free normalization, so a wrong target thickness would shift the absolute scale
by that factor and drive $\langle$data/OM$\rangle$ far from unity rather than to
$\approx0.9$. The sliced bound-state data inherit this same scale through
Eq.~\eqref{eq:znorm}: the $1/\Delta z$ factor rescales the full-detector
$N_{\rm target}$ to the slice, and for the $[0.9,1.0]$~m slice the rigorous
factor $\Delta z_{\rm full}/\Delta z_{\rm slice}=0.99/0.10=9.9$ differs from the
applied $1/0.1=10$ by only $1\%$. The transfer and elastic channels therefore
sit on one absolute scale to $\sim$1\,\%, well within the $\sim$12\,\%
optical-potential band. (The counts$\to$mb/sr conversion is the same
$N_{\rm beam}N_{\rm target}$ recipe as the transfer, defined in
\texttt{C16\_pd\_AngBins.C}; the solver \texttt{om\_elastic.py} and the elastic
data values \texttt{results/om\_elastic\_dd.csv} are version-controlled, so the
check is fully reproducible. Only the raw count-extract \texttt{16C\_dd\_gs.txt}
is uncommitted, and it is recoverable from those.)
...
\subsection{The earlier in-house \Cs\dpr\ analysis}
\label{sec:compare-movilla}

The present extraction supersedes, and is benchmarked against, the earlier
\Cs\dpr\ analysis of Movilla and Ayyad~\cite{MovillaAyyad2026}, whose reference
...
and seven unbound \Cse\ states from \Cs\dpr\ at $11.77$\mevu. The absolute scale
