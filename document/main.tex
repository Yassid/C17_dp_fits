...
(g.s.\ $3/2^+$, $0.217$~MeV $1/2^+$, $0.335$~MeV $5/2^+$) were digitized and
brought onto a common $\theta_{\rm c.m.}$ axis with the present data
(Fig.~\ref{fig:plboverlay}). FRESCO single-particle shapes were then scaled by
the published spectroscopic factors and compared to the efficiency-corrected
data, both state-by-state (Fig.~\ref{fig:plbpanels}) and as a coherent sum with
the paper's $C^2S$ applied to our DWBA (Fig.~\ref{fig:plbsfs}). The resulting
spectroscopic factors are compared in Table~\ref{tab:plbsf}.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.8\linewidth]{plb_overlay.png}
  \caption{Bound-state d$\sigma$/d$\Omega$: this experiment (black, $11.77$\mevu)
  and the digitized Pereira-Lopez \textit{et al.}\ PLB Fig.~2(a) data (red
  squares, $17.2$\mevu, $\theta_{\rm lab}\!\to\!\theta_{\rm c.m.}$) on a common
  axis, with the paper-$SF$ DWBA ($+$ background) curve.}
